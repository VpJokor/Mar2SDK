package com.mar2sdk.core.ad

// 广告配置
object AdKey {
	const val KEY_DEFAULT_PLATFORM = "mar2sdk.ad_config.defaultPlatform"
	const val KEY_ACTIVE_PLATFORMS = "mar2sdk.ad_config.activePlatforms"
	const val KEY_EXPLORE_PLATFORMS = "mar2sdk.ad_config.explorePlatforms"
	const val KEY_PRICE_MULTIPLIER = "mar2sdk.ad_config.priceMultiplier"
	const val KEY_PRICE_OFFSET = "mar2sdk.ad_config.priceOffset"
	const val KEY_IS_OPEN = "mar2sdk.ad_config.isOpen"
	const val KEY_SHOW_MAX_TIME = "mar2sdk.ad_config.showMaxTime"
	const val KEY_SHOW_MIN_TIME = "mar2sdk.ad_config.showMinTime"
	const val KEY_SHOW_MOD = "mar2sdk.ad_config.showMod"
	const val KEY_1H_MAX = "mar2sdk.ad_config.1HMax"
	const val KEY_24H_MAX = "mar2sdk.ad_config.24HMax"
	const val KEY_INTERVAL = "mar2sdk.ad_config.interval"
	const val KEY_AD_UNITS = "mar2sdk.ad_config.adUnits"
	const val KEY_AD_BANNER = "mar2sdk.ad_config.adBanner"
	const val KEY_AD_NATIVE = "mar2sdk.ad_config.adNative"
}

...
| `netEvents` | 字符串数组 | `["ad_revenue", "ad_impression", "ad_click"]` | 自有服务端广告事件上报。 |
...
`netEvents` 当前支持广告收入、展示和点击三个事件；其他事件即使配置了 `"*"` 也会跳过。已设置的空名单和自定义规则按原值使用。

网络广告采集从登录成功或恢复有效登录缓存后开始。需配置 `CommonConfig.serverAppID`、`serverClientKey` 和 `serverUrl`；默认随 `Core.init()` 自动登录，关闭 `isAutoLogin` 时需主动调用 `NetUtil.login()`。登录前的事件不会生成匿名账号或补记到后来的账号。

现有广告回调通过 `LogUtil.log` 自动进入 `logNet`，无需再次手动发送同一事件。采集时固定账号、时间、事件 ID、数数预置属性和广告参数，并在后台保存到独立 SQLite 队列。达到 `reportBatchSize` 条或等待 `reportFlushIntervalMillis` 毫秒后触发批量发送，默认 20 条或 5 秒；单条事件最多 64 KiB，每批事件 JSON 最多 256 KiB，实际批次也受此大小限制。
